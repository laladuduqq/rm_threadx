...
{
    (void)thread_input;
    SystemWatch_RegisterTask(&dmimuTask_thread, "dmimuTask");
    for (;;) 
    {
        SystemWatch_ReportTaskAlive(&dmimuTask_thread);
...
void DM_IMU_Init(TX_BYTE_POOL *pool)
{
    ...
    if (tx_byte_allocate(pool, (VOID **)&dmimu_thread_stack, 1024, TX_NO_WAIT) != TX_SUCCESS) {
        log_e("Failed to allocate stack for dmimuTask!");
        return;
    }

    UINT status = tx_thread_create(&dmimuTask_thread, "dmimuTask", DMimuTask, 0,dmimu_thread_stack, 1024, 7, 7, TX_NO_TIME_SLICE, TX_AUTO_START);
