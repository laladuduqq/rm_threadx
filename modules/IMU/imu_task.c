...
void INS_TASK_init(TX_BYTE_POOL *pool)
{
    ...
    if (tx_byte_allocate(pool, (VOID **)&ins_thread_stack, 1024, TX_NO_WAIT) != TX_SUCCESS) {
        log_e("Failed to allocate stack for insTask!");
        return;
    }

    UINT status = tx_thread_create(&INS_thread, "insTask", INSTask, 0,ins_thread_stack, 1024, 5, 5, TX_NO_TIME_SLICE, TX_AUTO_START);
